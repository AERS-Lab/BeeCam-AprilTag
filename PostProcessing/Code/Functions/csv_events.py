...
def csv_events(colony, events_by_ID):
    ...
    f_events_allRows = []
    f_events_titleRow = 'ID_num, num_of_detections, original_index_start, start_time, original_index_end, end_time, event_type_by_y_location, event_type_by_summed_vector_direction, event_result, summed_vector_angle_degrees, ,detections_index\n'      
    ...
    for u in range(0,len(events_by_ID)):
        if events_by_ID[u] != 'no detections for this ID':
            for v in range(0,len(events_by_ID[u])):
                if len(events_by_ID[u][v]) == 7: # there was an error with some mis-detected IDs not recording all info; filter these out
                    next_row = str(u)
                    ...
                    if events_by_ID[u][v][4] == 'enter' and events_by_ID[u][v][5][0] == 'enter':
                        next_row+= ',' + 'ENTER'
                    elif events_by_ID[u][v][4] == 'unknown' and events_by_ID[u][v][5][0] == 'enter':
                        next_row+= ',' + 'ENTER'
                    elif events_by_ID[u][v][4] == 'exit' and events_by_ID[u][v][5][0] == 'exit':
                        next_row+= ',' + 'EXIT'
                    elif events_by_ID[u][v][4] == 'unknown' and events_by_ID[u][v][5][0] == 'exit':
                        next_row+= ',' + 'EXIT'    
                    else:
                        next_row+= ',' + 'UNKNOWN'
                    ...
                    list_index = str(events_by_ID[u][v][6]).replace('[','')
