...
def sort_detect_by_ID(ID_num, center_x, center_y, angle, date_time, CPU_temp, max_ID):
    ...
    for ID in range(0,max(ID_num)+1):           ### instead max(ID_num) replace by max_ID othersiwe in case of a false ID with large number the list can be unnecessary long
        detections_by_ID.append([])
       
    # populate each ID's list with its detections
    for k in range(0,len(ID_num)):
        if ID_num[k] <= max_ID:
